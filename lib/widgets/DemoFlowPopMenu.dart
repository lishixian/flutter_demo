...
class _DemoFlowPopMenuState extends State<DemoFlowPopMenu>
  ...
  void initState() {
    ...
    _ctrlAnimationPopMenu.addListener(() => setState(() {
      print(" Listener");
    }));
  }
  ...
  Widget flowMenuItem(IconData icon) {
    ...
    return Padding(
      ...
      child: RawMaterialButton(
        ...
        child: Icon(icon, color: Colors.yellow, size: 30.0),
      ),
    );
  }
...
class FlowMenuDelegate extends FlowDelegate {
  ...
  void paintChildren(FlowPaintingContext context) {
    ...
    for (int i = 0; i < context.childCount; ++i) {
      x = (context.getChildSize(i)?.width ?? 10) * i * animation.value;
      //y = (context.getChildSize(i)?.width ?? 10) * i * animation.value;
      //print("paintChildren--$i-width:${context.getChildSize(i)?.width}--value:${animation.value}--x:$x--y:$y");
